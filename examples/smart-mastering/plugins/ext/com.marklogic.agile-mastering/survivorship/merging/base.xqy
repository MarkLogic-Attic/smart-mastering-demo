...
declare function merging:save-merge-models-by-uri(
  ...
  let $uris :=
    for $uri in $uris
    let $is-merged := xdmp:document-get-collections($uri) = "mdm-merged"
    return
      if ($is-merged) then
...
declare function merging:rollback-merge(
  ...
  return (
    ...
    let $new-collections := (
        xdmp:document-get-collections($previous-doc-uri)[fn:not(. = "mdm-archived")],
        "mdm-content"
      )
    return
    ...
    if ($retain-rollback-info) then (
      xdmp:document-set-collections($merged-doc-uri,
        (
          xdmp:document-get-collections($merged-doc-uri)[fn:not(. = "mdm-content")],
          "mdm-archived"
        )
      ),
...
declare function merging:get-options()
{
  cts:search(fn:collection(), cts:and-query((
      cts:collection-query("mdm-options"),
      cts:collection-query("mdm-merge")
    )))/merging:options
};
...
{
  fn:doc("/com.marklogic.agile-mastering/options/merging/"||$options-name||".xml")/merging:options
};
...
{
  xdmp:document-insert(
    "/com.marklogic.agile-mastering/options/merging/"||$name||".xml",
    $options,
    (xdmp:permission("mdm-admin", "update"), xdmp:permission("mdm-user", "read")),
    ("mdm-options", "mdm-merge")
  )
};

declare function merging:archive-document($uri as xs:string)
{
  xdmp:document-remove-collections($uri, "mdm-content"),
  xdmp:document-add-collections($uri, "mdm-archived")
};
...
declare function merging:get-option-names()
{
  ...
  let $option-names := $options ! fn:replace(
    fn:replace(., "/com.marklogic.agile-mastering/options/merging/", ""),
    ".xml", ""
  )
  return
