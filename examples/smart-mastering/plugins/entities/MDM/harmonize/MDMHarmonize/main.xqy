...
declare function plugin:main(
  ...
  $options as map:map)
{
  let $_raw-doc := map:put($options, "raw-document", fn:doc($id))
  ...
  let $_provenance :=
    for $prov-doc in auditing:auditing-receipts-for-doc-history($id)
    let $node-uri := xdmp:node-uri($prov-doc)
    return
      dhf:run-writer(
        xdmp:function(xs:QName("writer:write")),
        $node-uri,
        $prov-doc,
        map:entry("original-collections", xdmp:document-get-collections($node-uri))
      )
  return
