...
export class SmartMasteringDocViewerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const o1 = this.sm.getDoc(this.uri).pipe(tap(doc => {
      this.doc = this.formatDoc(xmlToJson(doc));
    }));
    ...
    const o2 = this.sm.getHistoryDocument(this.uri).pipe(
      tap(history => {
        ...
        console.log(['historyDocument', history]);
    ...
    const o3 = this.sm.getHistoryProperties(this.uri).pipe(tap(history => {
      ...
      console.log(['historyProperties', history]);
    }));
  ...
  render() {
    ...
    const nodeUris = activities.map((activity) => activity.resultUri);
    ...
    if (this.historyProperties) {
      ...
      activities.forEach((activity: any, activityIndex) => {
        ...
        if (!hasConnection) {
          ...
          if (connectionIndex > -1) {
            const linkedDocPropertyHistory = this.sm.getHistoryProperties(this.uri).pipe(
              tap(payload =>  {
      ...
      const margin = {top: 0, right: 50, bottom: 0, left: 50},
        ...
        height = 350;//window.innerHeight - margin.top - margin.bottom;
